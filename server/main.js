...
Meteor.startup(() => {
  ...
  Streamy.on("clientMove", (d, s) => {
    Streamy.broadcast(
      "movement",
      { data: { direction: d.data, id: d.id, x: d.data.x, y: d.data.y } },
      d.id
    );
  });
  ...
  Streamy.on("newChallenger", (d, s) => {
    ...
    Streamy.broadcast(
      "createChallenger",
      { challenger: { id: d.id, player: { x: d.player.x, y: d.player.y } } },
      d.id
    );
  });
  ...
  Streamy.on("createChallengerResponse", (d, s) => {
    Streamy.emit(
      "requestChallengers",
      {
        challenger: {
          id: d.id,
          player: { x: d.player.x, y: d.player.y, alive: d.player.alive }
        }
      },
      Streamy.sockets(d.newChallengerId)
    );
  });
  ...
  Streamy.on("bulletFire", (d, s) => {
    Streamy.broadcast(
      "spawnBullet",
      { data: { x: d.data.bulletx, y: d.data.bullety, facing: d.data.facing } },
      d.id
    );
  });
